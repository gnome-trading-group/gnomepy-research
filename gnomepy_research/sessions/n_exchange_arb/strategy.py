...
class _SpreadEWMA:
    def __init__(self, alpha: float = 0.99, warmup: int = 50):
        self.alpha = alpha
        self.warmup = warmup
        self._mean = 0.0
        self._var = 0.0
...
class NExchangeArb(Strategy):
    def __init__(
        ...
        self.same_exchange_only = same_exchange_only
    ...
    def _signed_spread(self, lst_a: Listing, lst_b: Listing) -> float:
        # Canonical spread: (mid_larger - mid_smaller), always from larger listing's perspective
    ...
    def _manage_open_pair(self, timestamp: int, stale: set[Listing]) -> list[Intent]:
        ...
        if self._closing:
            # Abandon residual after 30s — risk_averse partial fills can leave orders
            # stuck indefinitely; re-enable trading rather than blocking the session
    ...
    def _scan_entry(self, timestamp: int, stale: set[Listing]) -> list[Intent]:
        ...
        for i, lst_a in enumerate(live):
            for lst_b in live[i + 1:]:
                if self.same_exchange_only and lst_a[0] != lst_b[0]:
                    continue
                ...
                # Canonical spread: positive when larger listing is expensive
                spread = self._signed_spread(lst_a, lst_b)
                ...
                if z > self.entry_z and z > best_z:
                    if abs(spread) >= self.min_spread_bps:
                        # Larger listing is expensive: SELL larger, BUY smaller
                        best_z = z
                        best_long = larger
                        best_short = smaller
                        best_direction = 1
                    ...
                    if abs(spread) >= self.min_spread_bps:
                        # Smaller listing is expensive: SELL smaller, BUY larger
                        best_z = -z
                        best_long = smaller
                        best_short = larger
                        best_direction = -1
