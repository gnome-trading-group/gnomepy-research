...
class StablecoinStatArb(Strategy):
    def __init__(
        ...
        self._z_history: dict[tuple[Listing, Listing], deque] = {}
    ...
    def on_market_data(self, data: Schema) -> list[Intent]:
        ...
        if listing not in self._best_bid:
            ...
            for other in self._listings[:-1]:
                key = listing if listing > other else other
                ...
                if self.reversion_ticks > 0 and key not in self._z_history:
                    self._z_history[key] = deque(maxlen=self.reversion_ticks)
    ...
    def _manage_open_pair(self, timestamp: int, stale: set[Listing]) -> list[Intent]:
        ...
        if self._entering:
            ...
            if timestamp - self._entry_ts > self.entry_fill_timeout_ns:
                # Cancel any live passive before falling back to taker
                self._entering = False
                intents = []
                if qty_to_sell > 0:
                    intents.append(Intent(
                        exchange_id=long_lst[0], security_id=long_lst[1],
                        bid_price=0, bid_size=0, ask_price=0, ask_size=0,
                    ))
                    intents.append(self._take_qty(*long_lst, Side.ASK, qty_to_sell))
                if qty_to_buy > 0:
                    intents.append(Intent(
                        exchange_id=short_lst[0], security_id=short_lst[1],
                        bid_price=0, bid_size=0, ask_price=0, ask_size=0,
                    ))
        ...
        for i, lst_a in enumerate(live):
            for lst_b in live[i + 1:]:
                ...
                if self.reversion_ticks > 0:
                    hist = self._z_history.get(key)
                    if hist is None or len(hist) < self.reversion_ticks:
                        continue
                    peak_z = max(hist)
                    if abs_z >= peak_z:
                        continue
                ...
                if z > self.entry_z and abs_z > best_z:
                    best_z = abs_z
                    best_long = larger
                    best_short = smaller
                    best_direction = 1
                ...
                elif -z > self.entry_z and abs_z > best_z:
                    best_z = abs_z
                    best_long = smaller
                    best_short = larger
                    best_direction = -1
